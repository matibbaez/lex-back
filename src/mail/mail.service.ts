...
export class MailService {
  ...
  // --- ALERTA DE EXPEDIENTE INACTIVO ---
  async sendAlertaInactividad(email: string, nombreAbogado: string, caratula: string, diasSinVer: number) {
    try {
      const { data, error } = await this.resend.emails.send({
        from: 'TuLex Sistema <alertas@resend.dev>', // O tu dominio verificado
        to: [email],
        subject: `⚠️ Alerta: Expediente sin movimientos (${caratula})`,
        html: `
          <h1>Hola, ${nombreAbogado}</h1>
          <p>El sistema detectó que hace <strong>${diasSinVer} días</strong> no revisás el siguiente expediente:</p>
          <blockquote style="background: #f9f9f9; padding: 15px; border-left: 5px solid #ff0000;">
            <h3>${caratula}</h3>
          </blockquote>
          <p>Te recomendamos entrar al sistema y verificar si hubo novedades en el juzgado.</p>
          <br>
          <a href="http://localhost:5173/dashboard" style="padding: 10px 20px; background: #000; color: #fff; text-decoration: none; border-radius: 5px;">Ir al Sistema</a>
        `,
      });

      if (error) console.error('Error enviando alerta:', error);
    } catch (err) {
      console.error('Fallo crítico enviando mail:', err);
    }
  }
}